...
export default {
    ...
    effects: {
        * getList({page = 1}, {call, put, select}) {
            ...
            const filter = yield select(state => state.fund.time_filter)
            if (filter.filter) {
                post_data['sdate'] = filter.start
                post_data['edate'] = filter.end
            }
            const {data} = yield call(FundServices.getList, post_data)
            loading = false;
            if (data) {
                yield put({
                    type: 'assignList',
    ...
    reducers: {
        toggleDateFilter(state) {
            return {
                ...
                time_filter: {
                    ...state.time_filter,
                    visible: !state.time_filter.visible,
                }
            }
        },
        dateFilter(state, {start, end}) {
            return {
                ...state,
                time_filter: {
                    filter: true,
