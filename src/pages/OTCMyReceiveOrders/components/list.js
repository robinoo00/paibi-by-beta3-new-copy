...
class BuyList extends React.Component {
    ...
    render() {
        const hei = document.documentElement.clientHeight - 95;
...
const mapStateToProps = state => ({
    list: state.OTCMyReceiveOrders[state.OTCMyReceiveOrders.tabs.filter(item => item.choose)[0]['name']]
})

const mapDispatchToProps = dispatch => ({
    loadMore: () => {
        dispatch({
            type: 'OTCMyReceiveOrders/loadMore',
        })
    },
})
