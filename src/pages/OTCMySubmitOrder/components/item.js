...
const ItemFinish = ({item}) => (
    <div styleName="container" onClick={() => {router.push({pathname:'OTCSubmitOrderDetail',query:{no:item.接单单号}})}}>
        <Flex styleName="header">
            <div styleName="type-wrap">
                <span styleName="order-type" style={item.类型 == '购买' ? {backgroundColor:themes.BuyColor} : {backgroundColor:themes.SellColor}}>
                    {item.类型}
                </span>
            </div>
            <div styleName="name">
