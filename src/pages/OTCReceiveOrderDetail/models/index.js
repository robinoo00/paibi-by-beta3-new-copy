...
export default {
    namespace: 'OTCReceiveOrderDetail',
    state: {
        no: '',
        infos: [
    ...
    subscriptions: {
        setup({dispatch, history}) {
            return history.listen(({pathname, query}) => {
                if (pathname === '/OTCReceiveOrderDetail') {
                    dispatch({
                        type: 'assignNo',
                        no: query.no
                    })
    ...
    effects: {
        * getDetail({}, {call, select, put}) {
            const no = yield select(state => state.OTCReceiveOrderDetail.no)
        ...
        * shipments({},{call,select}){
            // const {data} = yield call(Services.shipments,{})
        },
        * shipments({},{call,select}){
            const order_data = yield select(state => state.OTCReceiveOrderDetail.data)
        ...
        * urge({}, {call, select}) {
            const order_data = yield select(state => state.OTCReceiveOrderDetail.data)
            const {data} = yield call(Services.urge,{no:order_data.接单单号,remarks:'123'})
            console.log(data)
            if(data){
                Modal.alert('',data.信息,[
                    {text:'我知道了',onPress:() => {
